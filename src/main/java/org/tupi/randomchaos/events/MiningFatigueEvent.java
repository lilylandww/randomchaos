...
public class MiningFatigueEvent implements ChaosEvent {
    private static final int DURATION_TICKS = 600;

    @Override
    public Identifier id() {
        return RandomChaosMod.id("mining_fatigue");
    }
    ...
    public void apply(ServerPlayer victim) {
        victim.addEffect(new MobEffectInstance(MobEffects.MINING_FATIGUE, DURATION_TICKS, 1));
        RandomChaosMod.LOGGER.info("Applied mining fatigue to {}", victim.getName().getString());
    }
}
