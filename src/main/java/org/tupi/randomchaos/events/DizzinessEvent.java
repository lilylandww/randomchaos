...
public class DizzinessEvent implements ChaosEvent {
    private static final int DURATION_TICKS = 600;

    @Override
    public Identifier id() {
        return RandomChaosMod.id("dizziness");
    }
    ...
    public void apply(ServerPlayer victim) {
        victim.addEffect(new MobEffectInstance(MobEffects.NAUSEA, DURATION_TICKS, 0));
        RandomChaosMod.LOGGER.info("Applied nausea (dizziness) to {}", victim.getName().getString());
    }
}
